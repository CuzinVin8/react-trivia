    ...
    let finalAs = props.allAs.map(Ans => {
        ...
        return (
            <li key={Ans}>
                ...
                <div className={newClass}><label htmlFor={props.newID[v]}>{Ans.replace(/&quot;/g, '"').replace(/&#039;/g, "'").replace(/&amp;/g, "&").replace(/&egrave;/g, "è").replace(/&rdquo;/g, '"').replace(/&ldquo;/g, '"').replace(/&aacute;/g, 'á').replace(/&ntilde;/g, 'ñ')}</label></div>
            </li>
        )
    })
    return (
